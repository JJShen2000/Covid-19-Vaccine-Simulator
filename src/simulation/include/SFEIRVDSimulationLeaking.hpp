...
protected:
    ...
    virtual void statisticUnit(const Time::TimeStep& ts, const std::vector<uint>& v2e, const std::vector<uint>& s2v, const std::vector<uint>& s2e, const std::vector<uint>& f2e, const std::vector<uint>& e2i, const std::vector<uint>& i2f, const std::vector<uint>& i2r, const std::vector<uint>& i2d) override {
        ...
        // for (auto v : v2e) if (ndp[v].stateID != 'E') std::cout << "GG\n";
        ...
        double p = ts.getDay() * ts.getPeriodLength() + ts.getPeriod() + 1;
        for (uint i = 0; i < N_lc; ++i) {
            for (uint j = 0; j < N_ag; ++j) {
                double sratio = scnt[i][j] / (double)allcnt[i][j];
                double fratio = fcnt[i][j] / (double)allcnt[i][j];
    ...
    std::vector<bool> have_infected, have_vaccined;
};
